...
if apt_install "nmap netcat-openbsd iproute2 dialog figlet wget curl python3 python3-pip python3-venv"; then
...
if [ "$NEED_VENV" -eq 1 ]; then
  ...
  if [ ! -d "$AI_ENV" ] || \
     [ ! -x "$AI_ENV/bin/python" ] || \
     [ ! -x "$AI_ENV/bin/pip" ] || \
     ! "$AI_ENV/bin/pip" show ultralytics >/dev/null 2>&1; then
    ...
    progress_bar 1 4 "Upgrading pip"
    "$AI_ENV/bin/pip" install --upgrade pip setuptools wheel >/dev/null 2>&1

    PIP_OK=1

    if [ "$INSTALL_AI_DETECT" -eq 1 ] || [ "$INSTALL_AI_TRAIN" -eq 1 ]; then
      progress_bar 2 4 "Installing torch (CPU)"
      "$AI_ENV/bin/pip" install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu --no-cache-dir >/dev/null 2>&1
      [ $? -ne 0 ] && PIP_OK=0
    fi

    progress_bar 3 4 "Installing AI packages"
    "$AI_ENV/bin/pip" install ultralytics opencv-python pillow numpy --no-cache-dir >/dev/null 2>&1
    [ $? -ne 0 ] && PIP_OK=0
    ...
    if [ "$PIP_OK" -eq 1 ]; then
      ...
      fail "Some AI packages failed to install"
      warn "Try manually: $AI_ENV/bin/pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu"
      warn "Then: $AI_ENV/bin/pip install ultralytics opencv-python pillow numpy"
    fi
...
if [ "$INSTALL_OFFENSIVE" -eq 1 ]; then
  ...
  if apt_install "nmap masscan aircrack-ng exploitdb hcxtools reaver hostapd dnsmasq bluez inotify-tools tor knockd fail2ban pandoc wkhtmltopdf"; then
...
if [ "$INSTALL_NEXUS" -eq 1 ]; then
  echo ""
  info "Installing NEXUS base dependencies"

  if apt_install "jq curl"; then
    ok "NEXUS base dependencies installed"
  else
    warn "Some NEXUS base dependencies failed"
  fi
  ...
  if command -v zerotier-one >/dev/null 2>&1; then
    ok "ZeroTier sudah terinstall, skipping"
  else
    curl -s https://install.zerotier.com | sudo bash >/dev/null 2>&1
    if command -v zerotier-one >/dev/null 2>&1; then
      ok "ZeroTier installed"
    else
      warn "ZeroTier install failed — cek koneksi internet"
    fi
  fi

  echo ""
  info "Configuring ZeroTier controller access..."

  ZT_HOME="/var/lib/zerotier-one"

  # Buat local.conf agar controller API bisa diakses lokal
  sudo bash -c "cat > $ZT_HOME/local.conf" <<'EOF'
{
  "settings": {
...
EOF
  ...
  if [ $? -eq 0 ]; then
    ok "ZeroTier local.conf configured"
  else
    warn "Gagal menulis local.conf — controller API mungkin tidak aktif"
  fi

  echo ""
  info "Enabling & starting ZeroTier service..."

  sudo systemctl enable zerotier-one >/dev/null 2>&1
  sudo systemctl restart zerotier-one
  timeout 10 bash -c 'until nc -z 127.0.0.1 9993; do sleep 1; done'

  # Verifikasi controller aktif
  progress_bar 1 3 "Waiting for ZeroTier"
  sleep 2
  progress_bar 2 3 "Verifying controller"

  ZT_TOKEN=$(sudo cat "$ZT_HOME/authtoken.secret" 2>/dev/null)
  CTRL_CHECK=$(curl -s -H "X-ZT1-Auth: $ZT_TOKEN" \
    http://127.0.0.1:9993/controller \
    --connect-timeout 5 2>/dev/null)
    
  progress_bar 3 3 "Done"  

  if echo "$CTRL_CHECK" | jq -e '.controller == true and .databaseReady == true'   >/dev/null 2>&1; then
    ok "ZeroTier controller aktif dan siap digunakan"
  else
    warn "Controller belum siap atau gagal diverifikasi"
  fi

  echo ""
  info "Installing NEXUS"

  mkdir -p "$HOME/.kecembung/nexus/networks"
  mkdir -p "$HOME/.kecembung/nexus/logs"

  progress_bar 1 3 "Downloading nexus.sh"
  curl -fsSL "$BASE_URL/kecembung_/nexus/nexus.sh" \
    -o "$HOME/.kecembung/nexus/nexus.sh" >/dev/null 2>&1

  if [ $? -eq 0 ] && [ -s "$HOME/.kecembung/nexus/nexus.sh" ]; then
    progress_bar 2 3 "Setting permissions"
    chmod +x "$HOME/.kecembung/nexus/nexus.sh"
    progress_bar 3 3 "Complete"
    ok "NEXUS installed"
  else
    progress_bar 3 3 "Failed"
    fail "Failed to download nexus.sh"
    warn "Run 'bung' then update to install NEXUS later"
  fi
fi
