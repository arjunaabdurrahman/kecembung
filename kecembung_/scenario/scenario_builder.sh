...
all_categories=(
"IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS" "IP TOOLS"
"TCP TOOLS" "TCP TOOLS" "TCP TOOLS" "TCP TOOLS" "TCP TOOLS" "TCP TOOLS" "TCP TOOLS" "TCP TOOLS" "TCP TOOLS"
"STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS" "STORAGE TOOLS"
"OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS" "OFFENSIVE TOOLS"
)

all_labels=(
"Lihat IP" "Scan Jaringan" "Ping IP" "Set IP Manual" "Routing" "DNS Check" "Internet Check" "Scan SSH" "SSH Connect" "Reset IP" "ARP Watch" "IP Blocklist" "Connection Monitor" "DHCP Snooping" "MAC Whitelist"
"TCP Scan Port" "TCP Service Detection" "TCP Range Scan" "TCP Specific Port" "TCP Local Ports" "TCP Banner Grab" "Port Knocking" "Fail2Ban Status" "SYN Flood Detect"
"Local Mode" "USB Mode" "Installer" "Save Log" "USB Detect" "USB Select" "USB Workspace" "USB Read" "USB Delete" "Log Integrity Check" "File Monitor" "Auto Report" "Telegram Exfil" "Scan Scheduler"
"Nmap Vuln Scan" "Masscan Aggressive" "Reverse Shell Gen" "WiFi Deauth" "Exploit Autopwn" "MAC Changer" "Logs Cleaner" "Torify Session" "Process Mask" "WiFi PMKID Attack" "Evil Twin AP" "WPS Pixie" "Bluetooth Scan"
)

all_commands=(
cmd_lihat_ip cmd_scan_jaringan cmd_ping_ip cmd_set_ip_manual cmd_routing cmd_dns_check cmd_internet_check cmd_scan_ssh cmd_ssh_connect cmd_reset_ip cmd_arp_watch cmd_ip_blocklist cmd_conn_monitor cmd_dhcp_snooping cmd_mac_whitelist
cmd_tcp_scan_port cmd_tcp_service_detection cmd_tcp_range_scan cmd_tcp_specific_port cmd_tcp_local_ports cmd_tcp_banner_grab cmd_port_knock_setup cmd_fail2ban_status cmd_syn_flood_detect
cmd_storage_local cmd_storage_usb cmd_storage_install cmd_storage_save_log cmd_usb_detect cmd_usb_select cmd_usb_workspace cmd_usb_read cmd_usb_delete cmd_log_integrity_check cmd_file_monitor cmd_auto_report cmd_telegram_exfil cmd_scan_scheduler
cmd_nmap_vuln_scan cmd_masscan_aggressive cmd_reverse_shell_gen cmd_wifi_deauth_attack cmd_exploit_autopwn cmd_mac_changer cmd_logs_cleaner cmd_torify_session cmd_proc_mask cmd_wifi_pmkid_attack cmd_evil_twin_ap cmd_wps_pixie cmd_bluetooth_scan
cmd_ai_chat_start cmd_ai_clear_history cmd_ai_delete_model cmd_ai_download_model cmd_ai_history cmd_ai_image cmd_ai_list_models cmd_ai_rtsp cmd_ai_status cmd_ai_video cmd_ai_webcam_all cmd_ai_webcam_person cmd_ai_run
)
...
for i in "${!all_commands[@]}"; do
  ...
  if [ "$cat" = "OFFENSIVE TOOLS" ] && [ "$OFFENSIVE_FLAG" -ne 1 ]; then
    continue
  fi
  ...
  case $mode in
    ...
    1)
      ...
      while true; do
        ...
        if [ -z "$cmd" ]; then
          echo -e "${YELLOW}[x] Input selesai${NC}"
          break
        fi
